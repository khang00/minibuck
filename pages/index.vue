<template>
  <v-row justify="space-around">
    <v-col v-for="(imagesInCol, i) in imagesForCols" :key="i" :cols="12/totalCols">
      <v-card v-for="(image, j) in imagesInCol" :key="j" class="mt-5" width="100%" height="auto">
        <v-img
          height="auto"
          width="100%"
          :src="image"
...
export default {
  ...
  data () {
    return {
      ...
      card: {
        image: 'https://i.pinimg.com/564x/c0/e6/8f/c0e68fb0427194738cb3d0b47c93e678.jpg',
        title: 'The title',
  ...
  computed: {
    imagesForCols () {
      ...
      for (let i = 0; i < this.images.length; ++i) {
        colsMatrix[i % 6].push(this.images[i])
      }
      return colsMatrix
    }
  }
